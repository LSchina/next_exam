package com.LSchina.exam.mapper;

import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.LSchina.exam.entity.Message;
...
public interface MessageMapper {
    ...
    @Results({
            @Result(property = "replays", column = "temp_id",many = @Many(select = "com.LSchina.exam.mapper.ReplayMapper.findAllById"))
    })
    IPage<Message> findAll(Page page);

    @Select("select id,title,content,time from message where id = #{id}")
    @Results({
            @Result(property = "replays", column = "id",many = @Many(select = "com.LSchina.exam.mapper.ReplayMapper.findAllById"))
    })
